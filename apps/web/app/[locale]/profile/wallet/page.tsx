    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t2("cancelWithdrawalDescription")}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export default function WalletPage() {
  ...
  const withdrawalPage = Math.max(
    1,
    Number(searchParams.get("withdrawals_page") || 1)
  )
  const withdrawalStatus = searchParams.get("withdrawals_status")
  const transactionPage = Math.max(
    1,
    Number(searchParams.get("transactions_page") || 1)
  )
  ...
  const withdrawalTotalPages =
    ...
    Math.max(
      1,
      Math.ceil((withdrawalMeta?.total ?? 0) / (withdrawalMeta?.limit ?? 5))
    )
  ...
  const transactionTotalPages =
    ...
    Math.max(
      1,
      Math.ceil((transactionMeta?.total ?? 0) / (transactionMeta?.limit ?? 10))
    )
  const walletPageHref = (
  ...
  const withdrawalFields: FormField<WithdrawalFormValues>[] = [
    {
      ...
      description: `${t("availableToWithdraw", { amount: money(availableBalance, data?.currency) })}${
        withdrawalExceedsBalance ? t("exceedsBalance") : ""
      }`,
      inputProps: {
        required: true,
        min: 1,
      ...
      <div className="flex flex-wrap gap-3">
        <Dialog>
          <DialogTrigger render={<Button />}>
            <HugeiconsIcon icon={PlusSignIcon} /> {t("addFunds")}
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
        ...
        <Dialog>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t("requestWithdrawalTitle")}</DialogTitle>
              <DialogDescription>
                {t("requestWithdrawalDescription")}
              </DialogDescription>
            </DialogHeader>
            <AppFormComplete
              form={withdrawalForm}
      ...
      <section className="space-y-4">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={walletPageHref({
                withdrawals_status: null,
                withdrawals_page: null,
              })}
            >
              <Badge
                variant={!withdrawalStatus ? "default" : "outline"}
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {date(item.requested_at ?? item.requestedAt)}
                </p>
              </div>
        ...
        <ListingPagination
          ...
          searchParams={{
            ...
            transactions_page:
              transactionPage > 1 ? String(transactionPage) : undefined,
          }}
      ...
      <section className="space-y-4 border-t pt-6">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={walletPageHref({
                ...
                transactions_page: null,
              })}
            >
              <Badge
                variant={!transactionStatus ? "default" : "outline"}
                className="cursor-pointer"
              >
                {t("all")}
              </Badge>
        ...
        <ListingPagination
          ...
          searchParams={{
            withdrawals_status: withdrawalStatus || undefined,
            withdrawals_page:
              withdrawalPage > 1 ? String(withdrawalPage) : undefined,
            transactions_status: transactionStatus || undefined,
          }}
