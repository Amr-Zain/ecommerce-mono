...
export class OrderLifecycleRepository {
  ...
  async cancelOrder(input: {
    ...
    const reservation = await this.prisma.$transaction(async (tx) => {
      ...
      await this.ordersRepository.lock(input.orderId, context);
      const order = await this.ordersRepository.findLifecycleOrder(input.orderId, context);
      ...
      await this.ordersRepository.updateOrder(order.id, { paymentStatus: PAYMENT_STATUSES.processingPayment }, context);
    ...
    if ((reservation as { allocated?: boolean }).allocated) {
      await this.processAllocatedCancellationRefund(
        reservation as Parameters<OrderLifecycleRepository['processAllocatedCancellationRefund']>[0],
      );
      return;
    }
    const gatewayReservation = reservation as Parameters<OrderLifecycleRepository['processCancellationRefund']>[1] & {
      attemptId: bigint;
    };
  ...
  async retryCancellationRefund(orderId: bigint, refundId: bigint, adminId?: bigint) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.ordersRepository.updateOrder(orderId, { paymentStatus: PAYMENT_STATUSES.processingPayment }, context);
    });
  ...
  async syncOrderPaymentStatus(tx: Prisma.TransactionClient, orderId: bigint) {
    const context = new PrismaTransactionContext(tx);
    const order = await this.ordersRepository.findOrderWithPayments(orderId, context);
    if (!order) throw new NotFoundException(this.i18n.t('errors.order_not_found'));
    const summary = this.paymentSummary(order.payments);
    const paymentStatus =
    ...
    await this.ordersRepository.updateOrder(orderId, { paymentStatus }, context);
    return summary;
  }
  ...
  async assertRemainingRefundCapacityTx(tx: Prisma.TransactionClient, orderId: bigint, amount: number) {
    const context = new PrismaTransactionContext(tx);
    await this.ordersRepository.lock(orderId, context);
    const order = await this.ordersRepository.findOrderWithPayments(orderId, context);
    if (!order) throw new NotFoundException(this.i18n.t('errors.order_not_found'));
    if (amount > this.paymentSummary(order.payments).remainingRefundableAmount) {
      throw new BadRequestException(this.i18n.t('errors.refund_amount_exceeds_remaining_paid_amount'));
  ...
  async createStatusHistory(
    ...
    const context = new PrismaTransactionContext(tx);
    const history = await this.ordersRepository.createStatusHistory(input, context);
    const order = await this.ordersRepository.findLifecycleOrder(input.orderId, context);
  ...
  private async processCancellationRefund(
    ...
    await this.prisma.$transaction(async (tx) => {
      const context = new PrismaTransactionContext(tx);
      const attempt = await this.paymentTransactionsRepository.findById(attemptId, context);
      if (!attempt) throw new NotFoundException(this.i18n.t('errors.order_cancellation_refund_not_found'));
      await this.ordersRepository.lock(attempt.orderId, context);
      const order = await this.ordersRepository.findLifecycleOrder(attempt.orderId, context);
      if (!order) throw new NotFoundException(this.i18n.t('errors.order_not_found'));
  ...
  private async finalizeCancellation(
    ...
    for (const item of order.items) {
      ...
      await this.variantsRepository.adjustStock(item.variantId, item.quantity, INVENTORY_REASONS.return, context);
    }
    ...
    const summary = this.paymentSummary(
      await this.paymentTransactionsRepository.findMany({ orderId: order.id }, context),
    );
