...
export function ApiAdvancedQuery() {
  return applyDecorators(
    ...
    ApiQuery({ name: 'filters[isActive]', required: false, example: '1', description: 'Nested filters (e.g. filters[isActive]=1)' }),
    ApiQuery({ name: 'sort[createdAt]', required: false, example: 'desc', description: 'Nested sort (e.g. sort[createdAt]=desc)' }),
    ApiQuery({ name: 'search', required: false, example: 'iphone', description: 'Search term' }),
    ApiQuery({ name: 'include', required: false, example: 'translations', description: 'Relations to include (comma-separated)' }),
  );
}
