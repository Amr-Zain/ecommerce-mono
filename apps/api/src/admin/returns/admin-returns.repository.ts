...
import { ReturnRequestsRepository } from '@/core/returns/return-requests.repository';
import { ExchangeRequestsRepository } from '@/core/returns/exchange-requests.repository';
...
type ReturnRequestWithRelations = Prisma.ReturnRequestGetPayload<{
  ...
  order: Prisma.OrderGetPayload<Record<string, never>> & {
    payments?: Prisma.PaymentTransactionGetPayload<Record<string, never>>[];
  };
  history?: Array<{
...
export class AdminReturnsRepository {
  constructor(
    ...
    @Inject(RETURN_REQUESTS_REPOSITORY) private readonly returnRequestsRepository: ReturnRequestsRepository,
    @Inject(EXCHANGE_REQUESTS_REPOSITORY) private readonly exchangeRequestsRepository: ExchangeRequestsRepository,
  ...
  async approveExchange(id: bigint, adminId?: bigint) {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      for (const [variantId, quantity] of requiredByVariant.entries()) {
        const reserved = await this.variantsRepository.reserveStock(
          variantId,
          quantity,
          INVENTORY_REASONS.reserve,
          context,
        );
        if (!reserved) {
  ...
  private async adjustStock(tx: Prisma.TransactionClient, variantId: bigint, quantity: number, reason: string) {
    await this.variantsRepository.adjustStock(variantId, quantity, reason, new PrismaTransactionContext(tx));
  }
  ...
  private formatReturnRequest(request: ReturnRequestWithRelations) {
    ...
    return {
      ...
      items: request.items.map((item) => ({
        ...
        productId: item.orderItem.productId?.toString() || null,
        productNameSnapshot: item.orderItem.productNameSnapshot,
        variantId: item.orderItem.variantId?.toString() || null,
        variantInfoSnapshot: item.orderItem.variantInfoSnapshot,
        imageSnapshot: item.orderItem.imageSnapshot,
  ...
  private formatExchangeRequest(request: ExchangeRequestWithRelations) {
    ...
    return {
      ...
      items: request.items.map((item) => ({
        ...
        newVariantSku: item.newVariant.sku,
        quantity: item.quantity,
        acceptedQuantity: item.acceptedQuantity,
