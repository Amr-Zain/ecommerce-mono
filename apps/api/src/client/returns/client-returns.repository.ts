...
type OrderItemForReturnExchange = OrderItemWithOrder;
...
@Injectable()
export class ClientReturnsRepository {
  constructor(
  ...
  private async lockAndAssertAvailableQuantities(
    ...
    for (const { orderItem, quantity } of items.sort((a, b) => Number(a.orderItem.id - b.orderItem.id))) {
      await this.ordersRepository.lockOrderItem(orderItem.id, new PrismaTransactionContext(tx));
