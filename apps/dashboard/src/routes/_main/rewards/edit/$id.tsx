...
import { earningRulesQueryKeys, rewardsQueryKeys } from '@/util/queryKeysFactory'
...
export const Route = createFileRoute('/_main/rewards/edit/$id')({
  loader: ({ context, params }) => {
    ...
    queryClient.ensureQueryData(
      prefetchOptions({
        queryKey: rewardsQueryKeys.getReward(params.id),
...
function RouteComponent() {
  ...
  const { data } = useFetch<ApiResponseBase<Reward>>({
    endpoint: `rewards/${id}`,
    queryKey: rewardsQueryKeys.getReward(id),
    suspense: true,
  })
