import { SmartBreadcrumbs } from '@/components/layout/SmartBreadcrumbs'
import PageForm from '@/components/pagesComponents/StaticPages/Form'
import { StaticPageShow } from '@/components/pagesComponents/StaticPages/show'
import { StaticPageShowSkeleton } from '@/components/pagesComponents/StaticPages/show/ShowSkeleton'
import useFetch from '@/hooks/UseFetch'
import { RouterContext } from '@/main'
import { ApiResponse } from '@/types/api/http'
import { StaticPage } from '@/types/api/staticPages'
import { prefetchOptions } from '@/util/preFetcher'
import { queryKeys } from '@/util/queryKeysFactory'
import { createFileRoute } from '@tanstack/react-router'

import { routePermission } from '@/lib/utils'

export const Route = createFileRoute('/_main/static-pages/show/$id')({
  beforeLoad: ({ context }) => {
    routePermission('static-pages', 'show')
    return context
  },
  component: RouteComponent,
  loader: async ({ params, context }) => {
    const { queryClient } = context as RouterContext
    queryClient.ensureQueryData(
      prefetchOptions({
        queryKey: queryKeys.pages.getPage(params.id),
        endpoint: `static-pages/${params.id}`,
      }),
    )
  },
  pendingComponent: StaticPageShowSkeleton,
})

function RouteComponent() {
  const { id } = Route.useParams()
  const { data } = useFetch<ApiResponse<StaticPage>, StaticPage>({
    queryKey: queryKeys.pages.getPage(id),
    endpoint: `static-pages/${id}`,
    suspense: true,
    select: (data) => data.data as unknown as StaticPage,
  })
  return (
    <>
      <SmartBreadcrumbs
        entityKey="menu.static-pages"
        entityTo="/static-pages"
        action="show"
      />
      <StaticPageShow page={data!} />
    </>
  )
}
