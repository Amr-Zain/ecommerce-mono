...
export const userColumns = (
  ...
  t: (key: string) => string,
): Array<ColumnDef<UserEntity>> => [
  imageColumn<UserEntity>('image', 'Form.labels.image'),
  ...
  textColumn<UserEntity>('user_type', 'Form.labels.user_type', {
    render: ({ row }) => (
      <Badge
        variant={
          row.original.user_type === 'client'
            ? 'default'
            : row.original.user_type === 'guest'
              ? 'secondary'
              : 'destructive'
        }
      >
  ...
  textColumn<UserEntity>('tier', 'Form.labels.tier', {
