...
import type { UserShow as UserShowType } from '../Config'
...
import { EmptyState, ShowHeader } from '@/components/common/show'
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <ShowHeader
        ...
        metaItems={[
          ...(user.last_login_at
            ? [
                {
                  value: `${t('userShow.last_login')} ${formatDate(user.last_login_at)}`,
                },
              ]
            : []),
        ]}
            ...
            {user.shopify_id && (
              <div className="flex items-center gap-1">
                <div className="text-sm font-medium">
                  {t('userShow.shopify_id')}:{' '}
                  {user.shopify_id || 'unregistered'}
                </div>
                <ButtonCopy
                  content={user.shopify_id || ''}
                  className="h-6 w-6"
                />
              </div>
            )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatsCard
          ...
          value={
            <div className="flex items-center gap-1">
              {user.statistics.total_spent} <SARIcon className="h-4 w-4" />
            </div>
          }
          icon={TrendingUp}
      ...
      <Card className="shadow-none">
        <CardHeader>
          ...
          <CardDescription>{t('userShow.loyalty_desc')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            />
            <StatsCard
              title={t('userShow.pending_points')}
              value={user.pending_points ?? 0}
              icon={CreditCard}
              iconColor="text-blue-600 bg-blue-500/10"
            />
            <StatsCard
              title={t('userShow.lifetime_points')}
          ...
          <div className="rounded-md border">
            <div className="flex items-center justify-between border-b px-3 py-2 text-sm">
              <span className="font-medium">{t('Form.labels.tier')}</span>
              <Badge
                style={
                  user.tier?.color
                    ? { backgroundColor: user.tier.color, color: '#111' }
                    : undefined
                }
              >
                {user.tier?.name || 'Bronze'}{' '}
                {user.tier?.multiplier ? `${user.tier.multiplier}x` : ''}
              </Badge>
            </div>
            <div className="divide-y">
              {user.loyalty_transactions.slice(0, 5).map((transaction) => (
                <div
                  key={transaction.id}
              ...
              {user.loyalty_transactions.length === 0 && (
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="shadow-none">
          <CardHeader>
            ...
            <CardDescription>
              {t('userShow.account_details_desc')}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
        ...
        <Card className="shadow-none">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2 border-b border-dashed">
              <span className="text-sm text-muted-foreground">
                {t('Form.labels.allow_notifications')}
              </span>
              <Badge
                variant={user.allow_notifications ? 'default' : 'secondary'}
              >
            ...
            {user.is_ban && (
              <div className="pt-4">
                <div className="text-xs font-semibold text-destructive uppercase mb-1">
                  {t('userShow.ban_reason')}
                </div>
            ...
            {user.devices.length > 0 ? (
              <div className="space-y-2">
                {user.devices.map((device) => (
                  <div
                    key={device.id}
                    className="text-sm p-3 bg-muted/50 rounded-md border flex items-center justify-between"
                  >
                    ...
                    <div className="text-right">
                      <Badge
                        variant="outline"
                        className="text-[10px] font-normal mb-1 block w-fit ml-auto"
                      >
                  ...
                  >
                    <div className="p-3 border rounded-md text-sm space-y-2">
                      <div className="flex items-start justify-between">
                        <div
                          className="font-medium text-xs line-clamp-1 pr-2"
